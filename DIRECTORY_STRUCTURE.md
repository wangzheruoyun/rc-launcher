...
> 自动生成的目录树（已排除 `rust/target`、`.cache`、各模块 `build`、`jniLibs`、
> `.gradle` 等生成/大目录，以及 `*.apk` 大二进制）。
>
> 说明：本项目是一个面向 Android 的 Minecraft Java 版启动器，采用
> **Rust 核心（后端/核心逻辑，通过 JNI 暴露为 `librc_launcher.so`）**
> + **Jetpack Compose（前端/UI）** 的分层架构，并针对中国大陆网络进行了
> 镜像源 / 断点续传 / DNS 优化。
>
> - `rust/`：Rust workspace，编译产物为 `librc_launcher.so`（cdylib）与可测试的
>   `rlib`；覆盖下载管理、网络优化、账号认证、JRE 供给、启动引擎、模组管理、
>   国际化、健壮性（重试/缓存/上报）等子系统。
> - `app/` + `core/` + `runtime/`：Android Gradle 模块（Compose UI、JNI 桥接、
>   预置 JRE 资源）。
> - `.github/workflows/`：CI（构建 / 发布 / 风格检查 / 健康审计）。

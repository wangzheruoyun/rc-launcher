...
Respect the seams in [`MODULES.md`](MODULES.md):
...
* For user-facing strings, add them to the i18n catalogue
  (`i18n.md`) rather than hard-coding text.
...
* Crash reports: include the Rust `CrashReporter` output and the JVM stack
  (see `launch/crash.rs` and `launch.md`).
...
[`BUILD.md`](BUILD.md), [`MODULES.md`](MODULES.md).
